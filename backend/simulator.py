...
def _apply_event(db: Session, tx: Transaction, current_state: str, new_state: str, event_type: str, source: str, sequence_num: int, latency: int = None, error_code: str = None):
    # Use State Machine to validate and generate event
    ...
    # Helper to convert to dict for broadcasting safely
...
async def run_scenario(db: Session, tx: Transaction, scenario: str):
    ...
    try:
        # Step 1: INITIATED (Common to all)
        ...
        if scenario == "success":
            ...
            current_state, event = _apply_event(db, tx, current_state, "TIMEOUT", "MERCHANT_RESPONSE_TIMEOUT", "AcquiringBank", seq, latency=4000, error_code="MCH_408")
            await manager.broadcast_transaction_update(tx.id, current_state, event)
            seq += 1
            await asyncio.sleep(0.5)
